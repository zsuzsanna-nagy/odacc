...
from dpn.expr import Expr, Fun, Var
...
class Encoding():
  ...
  # ------------------------------------------------------------------
  # Fast symbolic encoding for LIST-valued object guards
  # ------------------------------------------------------------------
  # The original encoder instantiates a LIST inscription with powerset(objects).
  # This is exact but exponential.  The encoding already contains optional
  # symbolic object slots for LIST inscriptions, so aggregate guards can be
  # expressed directly over those slots without enumerating concrete subsets.

  def _odacc_guard_object_decls(self, t):
    decls = {}
    for a in self._net._arcs:
      if a.get("source") != t["id"] and a.get("target") != t["id"]:
        continue
      for name, typ in a.get("inscription", []):
        if typ in self._net._data_types:
          continue
        if name not in decls or "LIST" in str(typ):
          decls[name] = typ
    return decls

  def _odacc_param_indices(self, t, logical_name):
    try:
      ps = self._net.object_inscriptions_of_transition(t, self._objects)
    except Exception:
      return []
    return sorted(set(int(p["index"]) for p in ps if p.get("name") == logical_name))

  def _odacc_concrete_property(self, prop_name, object_name, s, data_sub):
    """SMT term for prop_name(object_name), respecting ODACC soft semantics.

    JODAP patches the module-level ObjectPropertyReplacer with a soft-observation
    version.  The standalone SMT backend leaves the original hard replacement.
    Using that same symbol here preserves whichever semantics the caller chose.
    """
    try:
      term = Fun(prop_name, [Var(object_name, None)])
      replacer = ObjectPropertyReplacer(self._trace._objects)
      # ObjectPropertyReplacer normally replaces property expressions when they
      # occur as children of a comparison/binop.  Here the property term is the
      # root, so invoke its public replacement helper directly.  ODACC's soft
      # subclass overrides the same helper and therefore yields the matching
      # model-side soft variable instead of a hard observed constant.
      replaced = replacer.replace_arg(term) if hasattr(replacer, "replace_arg") else term
      return replaced.toSMT(s, data_sub)
    except Exception:
      return None

  def _odacc_scalar_property(self, t, i, logical_name, prop_name, decls, s, data_sub):
    typ = str(decls.get(logical_name, ""))
    if not typ or "LIST" in typ:
      return None
    idxs = self._odacc_param_indices(t, logical_name)
    if len(idxs) != 1:
      return None
    candidates = self._objects_by_type.get(typ, [])
    if not candidates:
      return None
    idx = idxs[0]
    value = s.real(0)
    for oname, oid in candidates:
      prop = self._odacc_concrete_property(prop_name, oname, s, data_sub)
      if prop is None:
        return None
      value = s.ite(s.eq(self._object_vars[i][idx], s.num(oid)), prop, value)
    return value

  def _odacc_list_property_sum(self, t, i, logical_name, prop_name, decls, s, data_sub):
    typ = str(decls.get(logical_name, ""))
    if "LIST" not in typ:
      return None
    base = typ[:typ.rfind(" LIST")]
    idxs = self._odacc_param_indices(t, logical_name)
    candidates = self._objects_by_type.get(base, [])
    if not idxs:
      return s.real(0)
    total = s.real(0)
    for oname, oid in candidates:
      # Logical LIST membership is represented by occurrence in any optional
      # object slot belonging to this inscription.  Summing per concrete object
      # keeps the aggregate set-like even if duplicate slot assignments are
      # temporarily possible before the marking constraints are asserted.
      used = s.lor([s.eq(self._object_vars[i][idx], s.num(oid)) for idx in idxs])
      prop = self._odacc_concrete_property(prop_name, oname, s, data_sub)
      if prop is None:
        return None
      total = s.plus(total, s.ite(used, prop, s.real(0)))
    return total

  def _odacc_fast_guard_expr(self, node, t, i, decls, s, data_sub):
    if node is None:
      return None
    cls = node.__class__.__name__
    if cls in ("Num", "Charstr", "Bool"):
      try:
        return node.toSMT(s, data_sub)
      except Exception:
        return None
    if cls == "Var":
      key = str(node)
      name = getattr(node, "name", None)
      if key in data_sub or name in data_sub:
        try:
          return node.toSMT(s, data_sub)
        except Exception:
          return None
      return None
    if cls == "BinOp":
      l = self._odacc_fast_guard_expr(node.left, t, i, decls, s, data_sub)
      r = self._odacc_fast_guard_expr(node.right, t, i, decls, s, data_sub)
    ...
    if cls == "Cmp":
      ...
      if l is None or r is None:
        return None
      if node.op == "==": return s.eq(l, r)
      if node.op == "!=": return s.neg(s.eq(l, r))
      if node.op == ">=": return s.ge(l, r)
      if node.op == "<=": return s.ge(r, l)
      if node.op == ">": return s.lt(r, l)
      if node.op == "<": return s.lt(l, r)
      return None
    if cls == "BinCon":
      l = self._odacc_fast_guard_expr(node.left, t, i, decls, s, data_sub)
      r = self._odacc_fast_guard_expr(node.right, t, i, decls, s, data_sub)
      if l is None or r is None:
        return None
      if node.op == "&&": return s.land([l, r])
      if node.op == "||": return s.lor([l, r])
      return None
    if cls == "Fun":
      name = getattr(node, "_name", None)
      args = list(getattr(node, "_args", []) or [])
      # Aggregate form used by the real-data benchmark:
      #     sum(price(I)), sum(weight(I)), ...
      if name == "sum" and len(args) == 1:
        inner = args[0]
        if inner.__class__.__name__ == "Fun":
          iargs = list(getattr(inner, "_args", []) or [])
          if len(iargs) == 1 and iargs[0].__class__.__name__ == "Var":
            logical = getattr(iargs[0], "name", None)
            if isinstance(logical, str) and "LIST" in str(decls.get(logical, "")):
              return self._odacc_list_property_sum(
                t, i, logical, getattr(inner, "_name", ""), decls, s, data_sub)
        return None
      # Scalar object property, e.g. price(o) or weight(p).
      if len(args) == 1 and args[0].__class__.__name__ == "Var":
        logical = getattr(args[0], "name", None)
        if isinstance(logical, str) and logical in decls and "LIST" not in str(decls[logical]):
          return self._odacc_scalar_property(t, i, logical, name, decls, s, data_sub)
      return None
    return None
  ...
  def transition_constraint(self, t,i):
    ...
    obj_inscrs = [ (v,tp) for (v,tp) in inscrs if tp not in self._net._data_types]
    plain_obj_vars = [ v for (v,tp) in obj_inscrs if "LIST" not in tp]
    list_obj_vars = [ v for (v,tp) in obj_inscrs if "LIST" in tp]
    ...
    # Exact O(n) symbolic aggregate encoding.  Fall back to CoCoMoT's generic
    # substitution encoder only for guard constructs this specialized translator
    # does not yet support.
    transition_decls = self._odacc_guard_object_decls(t)
    used_list_vars = [v for v in constr_vars if "LIST" in str(transition_decls.get(v, ""))]
    if used_list_vars:
      fast = self._odacc_fast_guard_expr(constr, t, i, transition_decls, s, sub)
      if fast is not None:
        self._odacc_list_guard_fast_paths = getattr(self, "_odacc_list_guard_fast_paths", 0) + 1
        avoided = 0
        for v in used_list_vars:
          typ = str(transition_decls[v])
          base = typ[:typ.rfind(" LIST")]
          n = len(self._objects_by_type.get(base, []))
          avoided += (1 << n) if n <= 62 else (1 << 62)
        self._odacc_list_guard_subsets_avoided = getattr(self, "_odacc_list_guard_subsets_avoided", 0) + avoided
        return fast
      self._odacc_list_guard_fallbacks = getattr(self, "_odacc_list_guard_fallbacks", 0) + 1

    # Generic exact fallback.
    substs = [[]]
    for v in constr_vars:
      if not "LIST" in inscr_types[v]:
        ...
        insts = powerset(objs)
      substsx = [ [(v,o)] + ss for ss in substs for o in insts]
      substs = substsx
    ...
    for subst in substs:
      ...
      for (v, obj) in subst:
        ...
        if "LIST" not in vtype:
          ...
          for (oname, oid) in obj:
            sub_conds.append(s.lor([s.eq(ovars[i][p["index"]],s.num(oid)) for p in used_vparams]))
      ...
      real_guard = sub_guard_smt if real_guard is None else s.ite(s.land(sub_conds), sub_guard_smt, real_guard)

    return real_guard if real_guard is not None else s.false()
