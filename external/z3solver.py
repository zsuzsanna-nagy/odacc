...
class Z3Solver(Solver):
  ...
  def mk_fun(self, name, args, osort=None):
    """Create or apply a function.

    CoCoMoT uses this method in two ways:
      1. mk_fun(name, input_sorts, output_sort) declares an uninterpreted
         function (used by the synthesis code);
      2. mk_fun(name, arguments) evaluates a parsed function expression
         (used by dpn.expr.Fun.toSMT).

    The original Z3 backend only implemented the first interface, while the
    Yices backend implements the second one. Supporting both keeps the old
    synthesis API intact and makes expressions such as sum(cost(P)) work
    with Z3 as well.
    """
    # Backwards-compatible function declaration interface.
    if osort is not None:
      sorts = args + [osort]
      return Function(name, *sorts)

    # Parsed/predefined function application interface.
    if name == "sum":
      # z3.Sum([]) returns 0, which is also the natural empty-sum value.
      return Sum(args)

    # Generic uninterpreted-function fallback. CoCoMoT currently treats
    # generic function expressions as numeric (the Yices backend uses Real).
    # Infer the argument sorts from the already translated expressions.
    if len(args) == 0:
      # There is no argument sort from which a result type can be inferred.
      # Keep the same numeric convention as the Yices backend.
      return Real(name)

    isorts = [arg.sort() for arg in args]
    result_sort = RealSort()
    fun = Function(name, *(isorts + [result_sort]))
    return fun(*args)
